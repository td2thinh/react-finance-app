import styles from "../style"
import { logo } from "../assets"
import { footerLinks, socialMedia } from "../constants"
const Footer = () => {
    return (
        <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
            <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
                <div className="flex-1 flex flex-col justify-start mr-10">
                    <img src={logo} alt="logo" className="w-[266px] h-[72px] object-contain" />
                    <p className={`${styles.paragraph} mt-5 max-w-[310px]`}>
                        A better way to enjoy every day.
                    </p>
                </div>

                <div className="flex-[1.5] w-full flex flex-row justify-between flex-wrap md:mt-0 mt-10">
                    {
                        footerLinks.map((item, index) => {
                            return (
                                <div key={index} className=" flex flex-col ss:my-0 my-4 min-w-[150px]">
                                    <h4>
                                        {item.title}
                                    </h4>
                                    <ul>
                                        {
                                            item.links.map((link, index) => {
                                                return (
                                                    <li key={link.name} className={`font-poppins font-normal text-[16px] leading-[24px] hover:bg-lightAccent cursor-pointer ${index !== item.links.length - 1 ? 'mb-4' : 'mb-0'}`}>
                                                        {link.name}
                                                    </li>
                                                )
                                            })
                                        }
                                    </ul>
                                </div>
                            )
                        })
                    }
                </div>
            </div>

            <div className="w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3f3r45]">
                <p className="font-poppins font-normal text-center text-[18px] leading-[27px]">
                    © 2023 Finance. All rights reserved
                </p>

                <div className="flex flex-row md:mt-0 mt-6">
                    {
                        socialMedia.map((item, index) => {
                            return (
                                <img key={item.id} src={item.icon} alt={item.id} className={`w-[24px] h-[24px] object-contain  cursor-pointer ${index !== socialMedia.length - 1 ? "mr-6" : "mr-0"}`} />
                            )
                        })
                    }
                </div>
            </div>
        </section>
    )
}

export default Footer